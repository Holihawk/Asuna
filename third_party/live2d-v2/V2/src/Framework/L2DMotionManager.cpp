...
int L2DMotionManager::startMotion(AMotion* motion, bool autoPriority) {
    ...
    for (auto& e : mMotions) {
        if (e.mFadeOut > 0) {
            double newEnd = now + e.mFadeOut * 1000.0;
...
bool L2DMotionManager::updateParam(ALive2DModel* model) {
    ...
    for (size_t i = 0; i < mMotions.size(); ) {
        ...
        // Differences of absolute times are taken in double and only then
        // narrowed: each result is a small relative quantity, so float is
        // plenty for it, but doing the subtraction in float would not be.
        float elapsed = (float)((now - e.mStartTimeMs) / 1000.0);

        // Fade-in weight
        float fadeIn = 1.0f;
        if (e.mFadeIn > 0 && e.mFadeInStartMs >= 0) {
            fadeIn = easeSine(
                (float)((now - e.mFadeInStartMs) / (e.mFadeIn * 1000.0)));
        }
        // Fade-out weight
        float fadeOut = 1.0f;
        if (e.mFadeOut > 0 && e.mEndTimeMs >= 0) {
            float remaining =
                (float)((e.mEndTimeMs - now) / (e.mFadeOut * 1000.0));
            if (remaining <= 0) {
                e.mFinished = true;
                fadeOut = 0;
            } else {
