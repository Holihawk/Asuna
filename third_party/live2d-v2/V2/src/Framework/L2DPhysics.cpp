...
// LOCAL PATCH (see ../../PATCHES.md §7). The per-object readers below replace
// two lambdas that located a value with `find('"', keyPos + 5)`. That offset is
// hard-coded for the 4-character key "id"; for "ptype" it lands on the key's own
// closing quote, so ptype parsed as ":" and every src and target fell through
// the `else { continue; }` branch below. Each hair chain ended up with no inputs
// and no outputs, and the whole physics system silently did nothing.
std::string jsonString(const std::string& obj, const char* key) {
    auto kp = obj.find(key);
    if (kp == std::string::npos) return "";
    auto col = obj.find(':', kp + std::strlen(key));
    if (col == std::string::npos) return "";
    auto q1 = obj.find('"', col);
    auto q2 = q1 != std::string::npos ? obj.find('"', q1 + 1) : std::string::npos;
    if (q2 == std::string::npos) return "";
    return obj.substr(q1 + 1, q2 - q1 - 1);
}

float jsonNumber(const std::string& obj, const char* key, float def) {
    auto kp = obj.find(key);
    if (kp == std::string::npos) return def;
    auto col = obj.find(':', kp + std::strlen(key));
    if (col == std::string::npos) return def;
    return (float)std::atof(obj.c_str() + col + 1);
}
        ...
        if (srcPos != std::string::npos) {
            ...
            if (sa != std::string::npos) {
                ...
                while (sp < srcArr.size()) {
                    ...
                    std::string tid = jsonString(srcObj, "\"id\"");
                    std::string ptype = jsonString(srcObj, "\"ptype\"");
                    float scale = jsonNumber(srcObj, "\"scale\"", 1.0f);
                    float weight = jsonNumber(srcObj, "\"weight\"", 1.0f);
                    PhysicsSrcType st;
        ...
        if (tgtPos != std::string::npos) {
            ...
            if (ta != std::string::npos) {
                ...
                while (tp < tgtArr.size()) {
                    ...
                    std::string tid = jsonString(tgtObj, "\"id\"");
                    std::string ttype = jsonString(tgtObj, "\"ptype\"");
                    float scale = jsonNumber(tgtObj, "\"scale\"", 1.0f);
                    float weight = jsonNumber(tgtObj, "\"weight\"", 1.0f);
