...
#  define SHADER_VER "#version 100\n"   // GLSL ES 1.00 - LOCAL PATCH
#else
...
GLuint DrawParamOpenGL::compileShader(GLenum type, const char* src) {
    // LOCAL PATCH (see ../../PATCHES.md): the version line and the feature
    // defines are prepended here so the premultiply step can be switched off
    // when the textures were premultiplied on upload (LAppModel.cpp).
    static const char* kFilter = getenv("ASUNA_TEXFILTER");
    const bool premultiplied = !(kFilter && std::string(kFilter) == "raw");
    std::string full = SHADER_VER;
    if (premultiplied) full += "#define PREMULTIPLIED_TEXTURES 1\n";
    full += src;
...
#if defined(LIVE2D_GLES)
        GLint srgb = -1;  // GL_FRAMEBUFFER_SRGB is desktop-only - LOCAL PATCH
#else
